...
const RULES = Object.assign({ midoriSlot: 'w1', holidaySlots: 2, deadlineDay: 15, silverWeekMinDays: 4, requirePin: 'admin' }, CONFIG.rules || {});
const needsPin = (s) => RULES.requirePin === 'all' || (RULES.requirePin === 'admin' && s.role === 'admin');
...
const deadlineOf = (ym, months) => months?.[ym]?.deadline || `${addMonths(ym, -1)}-${pad2(RULES.deadlineDay)}`;
const isPastDeadline = (ym, months) => dateKey(new Date()) > deadlineOf(ym, months);
...
const isSpecialWeekday = (k) => { const w = dowOfKey(k); return w >= 1 && w <= 5 && !isHolidayKey(k) && !!specialNameOf(k); };   // GW・年末年始の平日（個別設定）
...
function validate(months, staff, yms) {
  ...
  yms.forEach(ym => monthKeys(ym).forEach(k => {
    ...
    if (isTobanDay(k) && !tb.length) add('info', k, null, `${fmtMd(k)} 当番 未割当${isSpecialWeekday(k) ? `（別枠: ${specialNameOf(k)}・祝日・連休画面で個別設定）` : ''}`);
...
function draftMonth(months, staff, ym) {
  ...
  monthKeys(ym).forEach(k => {
    const dow = dowOfKey(k);
    // 平日当番（月〜木）
    if (isTobanDay(k) && !isSpecialWeekday(k) && !holdersOf(local, k, 'toban', staff).length) {
...
/* ───────── Excel（当番シフト：NMC当番シフト.xlsx と同じ列） ───────── */
function exportTobanSheet(ym, months, staff) {
  const { y, m } = parseYm(ym);
  const keys = monthKeys(ym);
  const nm = (k, code) => holdersOf(months, k, code, staff).map(id => shortName(staff.find(s => s.id === id)?.name)).join('・');
  const tanto = (k) => {
    const dow = dowOfKey(k);
    if (isWeekdayHoliday(k)) return [nm(k, 'h1'), nm(k, 'h2')].filter(Boolean).join('/');
    if (dow === 5 || dow === 6 || dow === 0) return [nm(k, 'w1'), nm(k, 'w2')].filter(Boolean).join('/');
    return nm(k, 'toban');
  };
  const header = ['日付', '曜日', '研修医', '担当', 'バックアップ'];
  const rows = keys.map(k => [`${m + 1}/${dayOfKey(k)}`, dowJP[dowOfKey(k)], '', tanto(k), holidayName(k) || '']);
  const ws = XLSX.utils.aoa_to_sheet([header, ...rows]);
  ws['!cols'] = [{ wch: 7 }, { wch: 5 }, { wch: 10 }, { wch: 16 }, { wch: 16 }];
  ...
  const font = { name: 'BIZ UDPGothic', sz: 11 };
  for (let c = 0; c < header.length; c++) {
    const h = ws[XLSX.utils.encode_cell({ r: 0, c })]; if (h) h.s = { font: { ...font, bold: true }, fill: { fgColor: { rgb: 'D9D9D9' } }, border, alignment: { horizontal: 'center' } };
  }
  keys.forEach((k, i) => {
    const dow = dowOfKey(k);
    const fill = (dow === 0 || isHolidayKey(k)) ? 'FF0000' : dow === 6 ? '0070C0' : null;
    for (let c = 0; c < header.length; c++) {
      const addr = XLSX.utils.encode_cell({ r: i + 1, c });
      if (!ws[addr]) ws[addr] = { t: 's', v: '' };
      ws[addr].s = { font: { ...font, color: fill ? { rgb: 'FFFFFF' } : undefined, bold: !!fill }, border, alignment: { horizontal: c <= 1 ? 'center' : 'left' }, ...(fill ? { fill: { fgColor: { rgb: fill } } } : {}) };
    }
  });
  const wb = XLSX.utils.book_new();
  XLSX.utils.book_append_sheet(wb, ws, `${y}年${m + 1}月`);
  XLSX.writeFile(wb, `NMC当番シフト_${y}${pad2(m + 1)}.xlsx`);
}
...
async function pinHash(sid, pin) {
  const buf = await crypto.subtle.digest('SHA-256', new TextEncoder().encode(`resident-roster:${sid}:${pin}`));
...
function GridView({ ym, setYm, months, staff, user, isAdmin, issues, actions, push }) {
  ...
  const onCell = async (s, k) => {
    ...
    if (isPastDeadline(ym, months)) { push(`締切（${fmtMd(deadlineOf(ym, months))}）を過ぎています。変更は管理者に依頼してください`, 'warn'); return; }
  ...
  return (
    <div className="page">
      <div className="toolbar">
        ...
        <span className="muted">提出締切 {fmtMd(deadlineOf(ym, months))}</span>
        ...
        {isAdmin && <Fragment>
          ...
          <button className="btn sm" onClick={() => exportTobanSheet(ym, months, staff)}>当番シフト Excel</button>
          <button className="btn sm" onClick={() => exportExcel(ym, months, staff)} title="氏名×日付の勤務表全体">全体 Excel</button>
      ...
      {!isAdmin && <p className="note">自分の行をタップして <span className="ng">✖</span>（入れない日）と、病院から指定された 当直・準夜・夜間初期・外サポ を入力してください（当直の翌日は自動で「明け」になります）。締切は {fmtMd(deadlineOf(ym, months))} です。</p>}
  ...
  return (
    <div className="page">
      <div className="toolbar">
        ...
        <span className="muted">{fmtYm(yms[0])} 〜 {fmtYm(yms[2])} の3か月（締切 {fmtMd(deadlineOf(yms[0], months))}）</span>
        <div className="grow" />
        {isAdmin && <Fragment>
...
function WeekdayPanel({ ym, setYm, months, staff, isAdmin, push }) {
  ...
  return (
    <div className="page">
      <div className="toolbar">
        <MonthNav ym={ym} setYm={setYm} />
        <span className="muted">月〜木 1日1人（金〜日は週末当番）　締切 {fmtMd(deadlineOf(ym, months))}</span>
        <div className="grow" />
        {isAdmin && <Fragment>
      ...
      <table className="list">
        ...
        <tbody>
          {rows.map(k => {
            ...
            return (
              <tr key={k}>
                <td className="date">{fmtMd(k)}{isSpecialWeekday(k) && <div><span className="pill warn">別枠 {specialNameOf(k)}・自動下書き対象外</span></div>}</td>
...
function HolidayPanel({ ym, setYm, months, staff, isAdmin, push }) {
  ...
  const items = useMemo(() => {
    const out = [];
    yms.forEach(y => monthKeys(y).forEach(k => {
      if (isWeekdayHoliday(k)) out.push({ k, kind: 'hol', codes: ['h1', 'h2'], label: holidayName(k), special: specialNameOf(k) });
      else if (isSpecialWeekday(k)) out.push({ k, kind: 'wd', codes: ['toban', null], label: '平日（連休中）', special: specialNameOf(k) });
    }));
  ...
  return (
    <div className="page">
      ...
      <p className="note">GW・年末年始の平日（連休中の平日）は自動下書きの対象外なので、ここで当番を個別に決めます。祝日は祝日①②、別枠の週末は週末①②を日ごとに割り当てます（通常の週末のように3日通しの制約はかけません）。{codeLabel(MIDORI_H)}・{codeLabel(MIDORI)}はみどり兼任（大村在住者）です。</p>
      {!items.length && <p className="note">この期間に祝日・別枠はありません。</p>}
      {items.length > 0 && (
        <table className="list">
          <thead><tr><th>日付</th><th>名称</th><th>①{MIDORI === 'w1' ? '（みどり）' : ''}／平日は当番</th><th>②{MIDORI === 'w2' ? '（みどり）' : ''}</th></tr></thead>
          <tbody>
            {items.map(it => {
              ...
              return (
                <tr key={it.k} className={it.special ? 'special' : ''}>
                  ...
                  <td>{c2 && (it.kind === 'wk' || RULES.holidaySlots >= 2) ? <PersonSelect value={v2} disabled={!isAdmin} people={people} statusOf={st(it.k, c2)} onChange={sid => writeEntries(entriesAssignDay(months, staff, it.k, c2, sid))} /> : <span className="muted">—</span>}</td>
                </tr>
              );
            })}
          </tbody>
...
function MyPanel({ ym, setYm, months, staff, user, actions, push }) {
  ...
  const locked = me.role !== 'admin' && isPastDeadline(ym, months);
  const onDay = async (k) => {
    if (locked) { push(`締切（${fmtMd(deadlineOf(ym, months))}）を過ぎています。変更は管理者に依頼してください`, 'warn'); return; }
    const st = stateOf(k);
    if (mode === 'gairai') {
  ...
  return (
    <div className="page">
      <div className="toolbar">
        ...
        <span className="muted">締切 {fmtMd(deadlineOf(ym, months))}</span>
        <div className="grow" />
        <button className="btn sm" onClick={() => setProfile(true)}>自分の設定</button>
        {locked && <span className="pill err">締切後（管理者のみ変更可）</span>}
        <button className={`btn sm ${md.submitted?.[me.id] ? 'accent' : 'primary'}`} disabled={locked} onClick={async () => { await patchMonth(ym, { submitted: { [me.id]: md.submitted?.[me.id] ? FV.delete() : Date.now() } }); push(md.submitted?.[me.id] ? '入力完了を取り消しました' : `${fmtYm(ym)} の入力を完了しました`, 'success'); }}>
      ...
      {mode === 'pref' && <p className="note">残った日が当番に入れる日です。すべて <span className="wish">○</span>（可）が初期値で、タップするたびに <span className="wish">◎</span>（とても希望）→ <span className="ng">✖</span>（できない）→ <span className="wish">○</span> と切り替わります。</p>}
...
/* ───────── 提出状況（管理者） ───────── */
function StatusPanel({ ym, setYm, months, staff }) {
  const keys = monthKeys(ym);
  const md = months[ym] || {};
  const people = activeIn(staff, ym);
  const cnt = (sid, f) => keys.filter(k => f(k)).length;
  const rows = people.map(s => ({
    s, done: !!md.submitted?.[s.id],
    gairai: cnt(s.id, k => getCode(months, k, s.id) === 'gairai'),
    fixed: cnt(s.id, k => ['tochoku', 'yakan', 'gsapo'].includes(getCode(months, k, s.id))),
    wish: cnt(s.id, k => isWish(months, k, s.id)), ng: cnt(s.id, k => isNg(months, k, s.id)),
  })).map(r => ({ ...r, touched: r.gairai + r.fixed + r.wish + r.ng > 0 }));
  ...
  return (
    <div className="page">
      <div className="toolbar">
        <MonthNav ym={ym} setYm={setYm} />
        <span className={`pill ${doneN === rows.length ? 'ok' : 'warn'}`}>入力完了 {doneN} / {rows.length} 名</span>
        <span className="muted">締切 {fmtMd(deadlineOf(ym, months))}{md.deadline ? '（変更済）' : ''}</span>
        <label className="muted">締切を変更 <input type="date" className="inp" value={deadlineOf(ym, months)} onChange={e => e.target.value && patchMonth(ym, { deadline: e.target.value })} /></label>
        {md.deadline && <button className="btn sm" onClick={() => patchMonth(ym, { deadline: FV.delete() })}>既定に戻す</button>}
        {isPastDeadline(ym, months) && <span className="pill err">締切後：レジデントは変更不可</span>}
      </div>
      {undone.length > 0 && <p className="note">未完了：{undone.map(r => r.s.name).join('・')}　（「何か入力あり」でも「入力完了」を押していない人は未完了です）</p>}
      <table className="list">
        <thead><tr><th>氏名</th><th>入力完了</th><th style={{ textAlign: 'right' }}>外来</th><th style={{ textAlign: 'right' }}>当直・夜間初期・準夜</th><th style={{ textAlign: 'right' }}>◎ 希望</th><th style={{ textAlign: 'right' }}>✖ 不可</th><th>希望する外勤先</th></tr></thead>
        <tbody>
          {rows.map(r => (
            <tr key={r.s.id}>
              <td>{r.s.name}</td>
              <td>{r.done ? <span className="pill ok">完了</span> : r.touched ? <span className="pill warn">入力途中</span> : <span className="pill err">未入力</span>}</td>
              <td className="num">{r.gairai}</td><td className="num">{r.fixed}</td><td className="num">{r.wish}</td><td className="num">{r.ng}</td>
              <td className="muted">{[wantsUrakami(r.s) ? '浦上' : '', r.s.omura ? 'みどり' : '', r.s.ueda ? '上田記念' : ''].filter(Boolean).join('・') || '—'}{r.s.profileDone ? '' : '（未設定）'}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
function StaffPanel({ staff, months, ym, user, isAdmin, push }) {
  ...
  return (
    <div className="page">
      ...
      <div style={{ overflowX: 'auto' }}>
        <table className="list">
          <thead><tr><th>氏名</th><th>浦上</th><th>みどり</th><th>上田記念</th><th>PIN</th><th>在籍</th><th>{fmtYm(ym)} チーム</th><th>権限</th><th></th></tr></thead>
          <tbody>
            {sorted.map(s => (
              <tr key={s.id} style={{ opacity: s.status === 'active' ? 1 : .5 }}>
                ...
                <td>{s.omura ? '○' : ''}</td>
                <td>{s.ueda ? '○' : ''}</td>
                <td>{s.pinHash ? <span className="pill ok">設定済</span> : <span className="pill">未設定</span>}</td>
      ...
      {editing && <StaffModal
        onResetPin={async () => { if (!confirm(`${editing.name} さんのPINをリセットしますか？ 次回ログイン時に新しいPINを設定してもらいます`)) return; await staffCol().doc(editing.id).update({ pinHash: FV.delete() }); push('PINをリセットしました', 'success'); setEditing(null); }}
...
function ProfileModal({ me, onClose, push }) {
  ...
  const save = async () => {
    const upd = { ...f, profileDone: true };
    if (newPin) { if (!PIN_RE.test(newPin)) { push('PINは数字4〜6桁です', 'warn'); return; } upd.pinHash = await pinHash(me.id, newPin); upd.pinSetAt = Date.now(); }
    await staffCol().doc(me.id).update(upd); push('保存しました', 'success'); onClose();
  };
  return (
    <div className="backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="field">PINを変更する場合のみ入力（数字4〜6桁）<input type="password" inputMode="numeric" pattern="[0-9]*" value={newPin} onChange={e => setNewPin(e.target.value)} placeholder="変更しない場合は空欄" /></div>
...
function StaffModal({ init, isSelf, lastAdmin, onSave, onClose, onDelete, hasHistory, onResetPin }) {
  ...
  return (
    <div className="backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="toolbar">
          {init.id && init.pinHash && <button className="btn sm" onClick={onResetPin}>PINをリセット</button>}
...
/* ───────── ログイン / ヘッダー ───────── */
function PinModal({ s, onDone, onClose }) {
  const setup = !s.pinHash;
  const [pin, setPin] = useState(''); const [pin2, setPin2] = useState(''); const [err, setErr] = useState(''); const [busy, setBusy] = useState(false);
  const submit = async () => {
    setErr('');
    if (!PIN_RE.test(pin)) { setErr('数字4〜6桁で入力してください'); return; }
    if (setup && pin !== pin2) { setErr('確認用のPINが一致しません'); return; }
    setBusy(true);
    try {
      const h = await pinHash(s.id, pin);
      if (setup) { await staffCol().doc(s.id).update({ pinHash: h, pinSetAt: Date.now() }); onDone(); }
      else if (h === s.pinHash) onDone();
      else setErr('PINが違います。忘れた場合は管理者にリセットを依頼してください');
  ...
  return (
    <div className="backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        <h3>{s.name}{setup ? '　PINを設定' : '　PINを入力'}</h3>
        <p className="note">{setup ? '初回ログインです。本人だけが入力できるように、数字4〜6桁のPINを決めてください。次回からこのPINでログインします。' : '本人確認のため、設定したPINを入力してください。'}</p>
        <div className="field">PIN<input type="password" inputMode="numeric" pattern="[0-9]*" autoFocus value={pin} onChange={e => setPin(e.target.value)} onKeyDown={e => e.key === 'Enter' && !setup && submit()} /></div>
        {setup && <div className="field">PIN（確認）<input type="password" inputMode="numeric" pattern="[0-9]*" value={pin2} onChange={e => setPin2(e.target.value)} onKeyDown={e => e.key === 'Enter' && submit()} /></div>}
        {err && <div className="issue error" style={{ marginBottom: 8 }}>{err}</div>}
        <div className="toolbar"><div className="grow" /><button className="btn" onClick={onClose}>戻る</button><button className="btn primary" disabled={busy} onClick={submit}>{setup ? '設定してログイン' : 'ログイン'}</button></div>
      </div>
    </div>
  );
}
    ...
    <div className="login">
      <div className="login-box">
        ...
        <div className="login-list">
          {sorted.map(s => (
            <button key={s.id} onClick={() => needsPin(s) ? setSel(s) : onLogin(s)}>
        ...
        <p className="muted" style={{ marginTop: 10 }}>{RULES.requirePin === 'all' ? '名前を選ぶとPINの入力（初回は設定）画面が出ます。' : RULES.requirePin === 'admin' ? '管理者は名前を選んだあとPINを入力します（初回は設定）。' : ''}</p>
      </div>
      {sel && <PinModal s={sel} onClose={() => setSel(null)} onDone={() => { const u = sel; setSel(null); onLogin(u); }} />}
    </div>
  );
}
...
/* ───────── App ───────── */
const TABS = [['my', 'マイ予定'], ['grid', '勤務表'], ['weekend', '週末当番'], ['weekday', '平日当番'], ['holiday', '祝日・連休'], ['stats', '集計'], ['staff', 'レジデント'], ['status', '提出状況']];
function App() {
  ...
  return (
    <Fragment>
      ...
      <nav className="tabs">{TABS.filter(([id]) => id !== 'status' || isAdmin).map(([id, l]) => <button key={id} className={`tab ${tab === id ? 'active' : ''}`} onClick={() => setTab(id)}>{l}</button>)}</nav>
      ...
      {tab === 'status' && isAdmin && <StatusPanel {...common} />}
    </Fragment>
  );
}
